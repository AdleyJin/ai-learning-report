import { useDateFormatter as $60f33508b4cd9d3b$export$85fd5fdf27bacc79 } from '../i18n/useDateFormatter.js';
import { useLocale as $2eb8e6d23f3d0cb0$export$43bb16f9c6d9e3f7 } from '../i18n/I18nProvider.js';
import { toCalendarDate, isSameYear } from '@internationalized/date';
...
function $e4b70edea91ee6b0$export$89757cae093b4b95(props, state) {
    let formatter = ($60f33508b4cd9d3b$export$85fd5fdf27bacc79)({
    ...
    if (state.minValue && minDate.compare(state.minValue) < 0) {
        ...
        if (state.maxValue && maxDate.compare(state.maxValue) > 0) maxDate = (toCalendarDate)(state.maxValue);
    }
    let years = [];
    let date = minDate;
    let value = 0;
    while(date.compare(maxDate) < 0){
        if ((isSameYear)(date, state.focusedDate)) value = years.length;
        years.push({
    ...
    let { locale: locale } = ($2eb8e6d23f3d0cb0$export$43bb16f9c6d9e3f7)();
    let ariaLabel = (useMemo)(()=>new Intl.DisplayNames(locale, {
            type: 'dateTimeField'
        }).of('year'), [
        locale
    ]);
